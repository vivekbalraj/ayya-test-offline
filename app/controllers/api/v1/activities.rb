    ...
    class Activities < Grape::API
      ...
      resource :activities do
        ...
        get "", root: :activities do
          ...
          response[:temples] = Temple.select('id, name, information, temple_type, village, district, latitude, longitude, founded_at, country, state, taluk, pincode, street_address, is_primary_thangal, is_book_read, book_month, is_published, priest_name, facebook_page_url, img1_file_name, img2_file_name, img3_file_name, views').includes(:events, :cars).where(id: temples).as_json(include: [:events, :cars], :methods => [:images, :thumb])
